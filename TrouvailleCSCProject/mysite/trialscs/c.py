...
def auth(request):
    if request.method == 'POST':
        ...
        if str(givenOTP) == str(OTP):
            cur.execute('''INSERT INTO userdetails (UserMailID,Username,Password) values(?,?,?)''',
                        (mailID, name, password))
            conn.commit()
            cur1.execute('''INSERT INTO userdetails (UserMailID,Username,Password) values(?,?,?)''',
                         (mailID, name, password))
            conn1.commit()
...
def flightsresults(request):
    if request.method == 'POST':
        ...
        querystring = {"departure_date": dateoftravel, "sid": "iSiX639", 'number_of_itineraries': 1000,
                       "adults": int(nop),
        ...
        headers = {
            ...
            'x-rapidapi-key': '' # Password for API
        }
        global data
        response = requests.get(url, headers=headers, params=querystring)
        data = response.json()
        global destinCity, destinAirport, destinCountry, destinCode, originCity, originAirport, originCountry, originCode
        destinCity = data['getAirFlightDepartures']['results']['result']['search_data']['search_0']['destination'][
            'city']
        destinAirport = data['getAirFlightDepartures']['results']['result']['search_data']['search_0']['destination'][
            'name']
        destinCountry = data['getAirFlightDepartures']['results']['result']['search_data']['search_0']['destination'][
            'country']
        destinCode = data['getAirFlightDepartures']['results']['result']['search_data']['search_0']['destination'][
            'code']
        ...
        originCountry = data['getAirFlightDepartures']['results']['result']['search_data']['search_0']['origin'][
            'country']
        ...
        for i in data['getAirFlightDepartures']['results']['result']['itinerary_data']:
            symbol = data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['price_details'][
                'display_symbol']
            airname = data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                'airline']['name']
            timeOfDept = \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                'departure']['datetime']['time_12h']
            timeOfArr = \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                'arrival']['datetime']['time_12h']
            Tdur = \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0']['info'][
                'duration']
            dateOfDept = \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
            ...
            fare = data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['price_details'][
                'display_total_fare']
            conf = \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0']['info'][
                'connection_count']
            aircraft = \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                "flight_data"]["flight_0"]["info"]["aircraft"]
            mid1 = ''
            if \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0']['info'][
                'connection_count'] == 1:
                ...
                data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                    ...
                    POD = \
                    data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                        'flight_data'][j]['departure']['airport']['code']
                    POA = \
                    data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                        'flight_data'][j]['arrival']['airport']['code']
                    mid1 = POA + str(',')
                    break
            elif \
            data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0']['info'][
                'connection_count'] == 2:
                POD = data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                    'flight_data']['flight_0']['arrival']['airport']['code']
                POA = data['getAirFlightDepartures']['results']['result']['itinerary_data'][i]['slice_data']['slice_0'][
                    'flight_data']['flight_1']['arrival']['airport']['code']
                mid1 = POD + str(', ') + POA + str(',')

            dict1 = {'symbol': symbol, 'airname': airname, 'timeofDept': timeOfDept, 'timeofar': timeOfArr,
                     'Tdur': Tdur, 'dateoDept': dateOfDept, 'dateofarr': dateOfArr, 'fare': fare, 'conf': conf,
                     'aircraft': aircraft, 'id': id, 'mid': mid1[:-1]}
            ...
            if id == 10:
                break
        global d3
        d3 = {'dep': departure, 'ar': arrival, 'date': str(dateoftravel2), 'cc': cabinclass, 'nop': nop,
              'placeofdep': originCity, 'placeofar': destinCity, 'lst': mainList}
        redict = {'dep': departure, 'ar': arrival, 'date': str(dateoftravel2), 'cc': cabinclass, 'nop': nop,
                  'placeofdep': originCity, 'placeofar': destinCity, 'lst': mainList, 'tf': totalFlights}
...
def flightchoice(request):
    if request.method == 'POST':
        ...
        if co == int(0):
            ...
            lst1, lst2, lst3, lst4 = [], [], [], []
            for j in \
            data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                'slice_data']['slice_0']['flight_data']:
                POD = \
                data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                    'slice_data']['slice_0']['flight_data'][j]['departure']['airport']['code']
                POA = \
                data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                    'slice_data']['slice_0']['flight_data'][j]['arrival']['airport']['code']
                t1 = \
                data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                    'slice_data']['slice_0']['flight_data'][j]['departure']['datetime']['time_12h']
                t2 = \
                data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                    'slice_data']['slice_0']['flight_data'][j]['arrival']['datetime']['time_12h']
                dur = \
                data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                    'slice_data']['slice_0']['flight_data'][j]['info']['duration']
                T1 = \
                data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                    'slice_data']['slice_0']['flight_data'][j]['departure']['datetime']['date']
                T2 = \
                data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                    'slice_data']['slice_0']['flight_data'][j]['arrival']['datetime']['date']
                lst1.append(POD)
                lst1.append(POA)
                lst2.append(t1)
                lst2.append(t2)
                ...
                lst4.append(T1)
                lst4.append(T2)
            d2 = {'dep': lst1[0], 'mid': lst1[1], 'arr': lst1[3], 't1': lst2[0], 't2': lst2[1], 't3': lst2[2],
                  't4': lst2[3], 'dur1': lst3[0], 'dur2': lst3[1], 'T1': lst4[0], 'T2': lst4[1], 'T3': lst4[2],
                  'T4': lst4[3]}
            dict2.update(d2)
            ...
            data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                ...
                lst4.append(T1)
                lst4.append(T2)
            d2 = {'dep': lst1[0], 'mid1': lst1[1], 'mid2': lst1[3], 'arr': lst1[5], 't1': lst2[0], 't2': lst2[1],
                  't3': lst2[2], 't5': lst2[4], 't6': lst2[5],
                  't4': lst2[3], 'dur1': lst3[0], 'dur2': lst3[1], 'dur3': lst3[2], 'T1': lst4[0], 'T2': lst4[1],
                  'T3': lst4[2],
    ...
    if request.method == 'POST':
        ...
        '''
            ...
            2 stops -
                        ...
                        '''
        if dict2['conf'] == 0:
            fno = data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                'slice_data']['slice_0']['flight_data']['flight_0']['info']["flight_number"]
            ...
            Tdur = data['getAirFlightDepartures']['results']['result']['itinerary_data']['itinerary_' + str(int(fav))][
                'slice_data']['slice_0']['info']['duration']
            ...
            cur1.execute('''INSERT INTO FlightDataStorage(BookingRefNumber,UserMailID,UserName,DateofSearch,PassengerCount,DepartureLOC,DateOfDeparture,ArrivalLOC,DateOfArrival,CabinClass,Airline,TotalFare,Duration,Currency,ConnectingFlights,StatusR,TimeOfDept,TimeOfArr) 
                                                        Values('{}','{}','{}','{}',{},'{}','{}','{}','{}','{}','{}',{},'{}','{}',{},'{}','{}','{}')'''.format(
                bookingrefnum, entuser, entuser[:-10], date, 1, dict2['dep'], str(dict2['dateoDept']), dict2['ar'],
                str(dict2['dateofarr']), cabinclass, dict2['airname'] + ' ' + str(fno), dict2['fare'], str(Tdur),
                convert_currency, 0, 'Success', str(dict2['timeofDept']), str(dict2['timeofar'])))
            conn1.commit()
            ...
            emc3 = ' \nDate of Departure: {}\n Estimated time of Departure: {}\n Date of Arrival: {}\n Estimated time of Arrival {}\nNumber of persons travelling: 1 \n Total Fare: {} \nConnecting Flights: 0\n\n'.format(
                str(dict2['dateoDept']), dict2['timeofDept'], str(dict2['dateofarr']), dict2['timeofar'],
                str(convert_currency) + ' ' + str(dict2['fare']))
            emc4 = 'Your Journey:\n\n {} ---> {} \n Airline: {} \n Cabin Class: {}\n Estimated departure from {}: {}\n Estimated arrival at {}: {}\nDuration of Flight:{} \n\n\nKindly find attatched the e-ticket.\nWishing you a safe journey!\nWith regards,\nTrouvaille Inc.'.format(
                str(originCode), str(destinCode), dict2['airname'] + ' ' + str(fno), cabinclass, str(originCode),
                dict2['timeofDept'], str(destinCode), dict2['timeofar'], Tdur)
            ...
            emc3 = ' \nDate of Departure: {}\n Estimated time of Departure: {}\n Date of Arrival: {}\n Estimated time of Arrival {}\nDuration of flight: {}\nNumber of persons travelling: 1 \n Total Fare: {} \nConnecting Flights: 1 Cabin Class: {}\n\n'.format(
                str(dict2['T1']), str(dict2['t1']), str(dict2['T4']), str(dict2['t4']), str(Tdur),
                str(convert_currency) + ' ' + str(dict2['fare']), cabinclass)
            emc4 = '\n\nYour Journey:\n\n'
            emc5 = '{} --> {}\nEstimated time of departure from {}: {}\nEstimated time of arrival at {}: {}'.format(
                str(originCode), str(dict2['mid']), str(originCode), str(dict2['t1']), str(dict2['mid']),
                str(dict2['t2']))
            emc6 = '\n\n{} --> {}\nEstimated time of departure from {}: {}\nEstimated time of arrival at {}: {}'.format(
                str(dict2['mid']), str(destinCode), str(dict2['mid']), str(dict2['t3']), str(destinCode),
                str(dict2['t4']))
            emc7 = '\n\n\nKindly find attatched the e-ticket.\nWishing you a safe journey!\nWith regards,\nTrouvaille Inc.'
